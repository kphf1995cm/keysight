...
//Output tuple
//
void ShowPacketAndTuple(Ptr<Node> s, bool isCount)
{
...
// Output keysight
//
void ShowKeysight(Ptr<Node> s, bool isCount)
{
...
main(int argc, char *argv[])
{
	...
	unsigned int micePackets = 80;// mice flow 10KB (80% quantity but only occupys 20% flow sum)
	unsigned int elephantPackets = 1280; //elephant flow 160KB (20% quantity but occupys 80% flow sum)
	...
	for (i = 0; i < num_group; i++)
	{
		for (j = 0; j < num_core; j++)
		{
			...
			ShowPacketAndTuple(core[i].Get(j),true);
			ShowKeysight(core[i].Get(j),true);
		}
	}
	...
	for (i = 0; i < num_pod; i++)
	{
		for (j = 0; j < num_agg; j++)
		{
			...
			ShowPacketAndTuple(agg[i].Get(j),true);
			ShowKeysight(agg[i].Get(j),true);
		}
	}
	...
	for (i = 0; i < num_pod; i++)
	{
		for (j = 0; j < num_edge; j++)
		{
			...
			ShowPacketAndTuple(edge[i].Get(j),true);
			ShowKeysight(edge[i].Get(j),true);
		}
	}
	...
	for (i = 0; i < num_pod; i++)
	{
		for (j = 0; j < num_edge; j++)
		{
			for (k = 0; k < num_host; k++)
			{
				...
				ShowPacketAndTuple(host[i][j].Get(k),false);
				ShowKeysight(host[i][j].Get(k),false);
			}
		}
	}
	std::cout << "==========================================================" << std::endl;
	...
	std::cout << "Pair Num:" << pairNum << std::endl;
	std::cout << "Route Protocol Type:";
	if (routeProtocolType == 0)
		std::cout << "Nix Vector" << std::endl;
	else
		std::cout << "ECMP" << std::endl;
	std::cout << "Flow Distribution Type:";
	switch (flowType)
	{
	case 0:std::cout << "exponential" << std::endl; break;
	case 1:std::cout << "normal" << std::endl; break;
	case 2:std::cout << "constant" << std::endl; break;
	default:std::cout << "28distribution" << std::endl; break;
	}
	std::cout << "Min Packets Num:" << minPackets << std::endl;
	std::cout << "Max Packets Num:" << maxPackets << std::endl;
	ShowDifference();
